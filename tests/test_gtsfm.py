...
class TestGTSFM(unittest.TestCase):
    ...
    def __assert_point3_equal_upto_scale(self,
                                         ...
                                         rot_list1: Optional[Rot3],
                                         rot_list2: Optional[Rot3]):
        ...
        for i in range(len(wTi_list1)):
            if wTi_list1[i] is None:
                ...
                direction1 = Unit3(rot_list1.rotate(
                    wTi_list1[i] - wTi_list1[reference_idx]))
                direction2 = Unit3(rot_list2.rotate(
                    wTi_list2[i] - wTi_list2[reference_idx]))

                self.assertTrue(direction1.equals(direction2, 1e-2))

    def __assert_pose3_equal_upto_scale(self,
                                        wPi_list1: List[Optional[Pose3]],
                                        wPi_list2: List[Optional[Pose3]]):
        """Helper function to assert that two lists of global Point3 are equal
        (upto global scale ambiguity)."""
        ...
        # assert length of both the lists
        self.assertEqual(len(wPi_list1), len(wPi_list2))

        # select the first valid rotation entry to tackle global ambiguity
        reference_idx = -1
        for i in range(len(wPi_list1)):
            if wPi_list1[i] is not None:
                self.assertIsNotNone(wPi_list2[i])
                reference_idx = i
                break

        if reference_idx == -1:
            # confirm the 2nd list has all Nones too
            for val in wPi_list2:
                self.assertIsNone(val)
        ...
        # compare all translations w.r.t the reference_idx
        for i in range(len(wPi_list1)):
            if wPi_list1[i] is None:
                self.assertIsNone(wPi_list2[i])
            else:
                pose1 = wPi_list1[i].between(wPi_list1[reference_idx])
                pose2 = wPi_list2[i].between(wPi_list2[reference_idx])

                direction1 = Unit3(pose1.translation())
                direction2 = Unit3(pose2.translation())
    ...
    def test_create_computation_graph(self):
        ...
        # form global poses
        computed_global_poses = [None]*len(computed_global_rotations)
        expected_global_poses = [None]*len(expected_global_rotations)

        for i in range(len(computed_global_rotations)):
            if computed_global_translations[i] is not None:
                computed_global_poses[i] = Pose3(
                    computed_global_rotations[i],
                    computed_global_translations[i])

            if expected_global_translations[i] is not None:
                expected_global_poses[i] = Pose3(
                    expected_global_rotations[i],
                    expected_global_translations[i])

        self.__assert_pose3_equal_upto_scale(
            computed_global_poses, expected_global_poses)

        # # get one pair of global rotations to align coordinate systems
        # reference_idx = -1
        # for i in range(len(computed_global_rotations)):
        #     if computed_global_rotations[i] is not None:
        #         reference_idx = i
        #         break

        # self.__assert_point3_equal_upto_scale(
        #     computed_global_translations, expected_global_translations,
        #     computed_global_rotations[reference_idx],
        #     expected_global_rotations[reference_idx]
        # )
