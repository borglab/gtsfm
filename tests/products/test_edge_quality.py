...
def test_is_bad_inf_error():
    ...
    score = EdgeQualityScore(num_supporting_tracks=0, mean_reproj_error_px=float("inf"), max_reproj_error_px=float("inf"))
    assert score.is_bad()
...
def test_is_bad_default_thresholds():
    ...
    good = EdgeQualityScore(num_supporting_tracks=5, mean_reproj_error_px=2.0, max_reproj_error_px=3.0)
    assert not good.is_bad()

    bad = EdgeQualityScore(num_supporting_tracks=1, mean_reproj_error_px=6.0, max_reproj_error_px=8.0)
    assert bad.is_bad()
