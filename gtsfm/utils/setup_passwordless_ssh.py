...
def setup_passwordless_ssh(cluster_config_path, username):
    ...
    for source_machine in machines:
        ...
        for target_machine in machines:
            ...
            copy_cmd = f"ssh -t {username}@{source_machine} 'ssh-copy-id -o StrictHostKeyChecking=no {username}@{target_machine}'"
    ...
    for source_machine in machines:
        for target_machine in machines:
            test_cmd = f"ssh {username}@{source_machine} 'ssh -o BatchMode=yes -o ConnectTimeout=5 {username}@{target_machine} echo SUCCESS' 2>/dev/null"
