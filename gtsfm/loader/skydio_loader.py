"""Simple loader class that reads any of Skydio's datasets from a folder on disk.
...
class SkydioLoader(LoaderBase):
    ...
    def get_image(self, index: int) -> Image:
        ...
        print(io_utils.load_image(self._image_paths[index]))
        image = io_utils.load_image(self._image_paths[index])
        print(image.exif_data)
        image.modify_exif_data("FocalLength", 3.7)
        print(image.exif_data)
        return image
    ...
    def get_camera_intrinsics(self, index: int) -> Cal3Bundler:
        ...
        if not self._use_gt_intrinsics:
            ...
            image = io_utils.load_image(self._image_paths[index])
            image.modify_exif_data("FocalLength", 3.7)
            intrinsics = image.get_intrinsics_from_exif()
