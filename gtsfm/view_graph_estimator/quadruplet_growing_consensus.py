...
class SpanningTreeViewGraphEstimator(ViewGraphEstimatorBase):
    ...
    def run(
        ...
        for (i0, i1, i2, i3) in quadruplets:
            num_inliers_cycle = (
                two_view_reports[tuple(sorted([i0, i1]))].num_inliers_est_model
                 + two_view_reports[tuple(sorted([i1, i2]))].num_inliers_est_model
                 + two_view_reports[tuple(sorted([i2, i3]))].num_inliers_est_model
                 + two_view_reports[tuple(sorted([i0, i3]))].num_inliers_est_model
            )
            num_inliers_all_cycles.append(num_inliers_cycle)

        sorted_idxs = np.argsort(-1 * np.array(num_inliers_all_cycles))
        sorted_cycles = [quadruplets[idx] for idx in sorted_idxs]
        
        quadruplet_is_used = np.zeros(len(quadruplets), dtype=bool)
        ...
        for quadruplet_cycle_error_threshold in [1.5, 2.5]:#, 5.0]:#, 7.5]:
            ...
            for epsilon in [0.5, 1.5]: # 0.75, 1.0, 1.25, 1.5, 2, 2.5, 3, 3.5,
                ...
                for quadruplet_idx, (i0, i1, i2, i3) in enumerate(sorted_cycles):
                    ...
                    if len(clean_G) > 0 and not any(i in clean_G.nodes() for i in [i0,i1,i2,i3]):
                    ...
                    quadruplet_cycle_error = compute_cycle_error(ordered_cycle_nodes=[i0,i1,i2,i3], i2Ri1_dict=i2Ri1_dict)

                    #  = comp_utils.compute_cyclic_quadruplet_rotation_error(
                    #     i1Ri0=i2Ri1_dict[(i0, i1)], i2Ri1=i2Ri1_dict[(i1, i2)], i2Ri0=i2Ri1_dict[(i0, i2)]
                    # )
                    gt_errors = [
                        two_view_reports[tuple(sorted([i0,i1]))].R_error_deg,
                        two_view_reports[tuple(sorted([i1,i2]))].R_error_deg,
                        two_view_reports[tuple(sorted([i2,i3]))].R_error_deg,
                        two_view_reports[tuple(sorted([i0,i3]))].R_error_deg
                    ]
                    print(f"Propose:cycle_error={quadruplet_cycle_error:.2f} vs. GT errors={np.round(gt_errors,2)} for quadruplet ({i0},{i1},{i2},{i3}), @ index={quadruplet_idx}")
                ...
                percent_quadruplets_used = quadruplet_is_used.mean() * 100
                print(f"% quadruplets used: {percent_quadruplets_used:.2f} % w/ epsilon={epsilon}")
        ...
        for clean_graph_idx, clean_graph in enumerate(clean_graphs):
            ...
            print(f"Tree {clean_graph_idx}: ", clean_graph.nodes())
...
def compute_tree_average_edge_error(sorted_edges, two_view_reports):
    errors = []
    for edge in sorted_edges:
        error = two_view_reports[edge].R_error_deg
        errors.append(error)
    print(f"\tAverage tree error: {np.mean(errors):.2f}")
    print(f"\tMax tree error: {np.amax(errors):.2f}")
