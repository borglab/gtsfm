...
def compare_metrics(txt_metric_paths: Dict[str, str], json_path: str) -> None:
    ...
    for pipeline_name in txt_metric_paths.keys():
        ...
        gtsfm_metrics.append(GtsfmMetric('Number of 3D Points ' + pipeline_name,
                                         data[2][data[2].find(":") + 1: data[2].find(",")]))
        gtsfm_metrics.append(GtsfmMetric('Mean Track Length ' + pipeline_name, data[2][data[2].rindex(":") + 1:]))
        gtsfm_metrics.append(GtsfmMetric(
        ...
        with open(fpath, "r") as f:
            data = f.readlines()
            images = data[4:]
        gtsfm_metrics.append(GtsfmMetric('Number of Images ' + pipeline_name,
                                         data[3][data[3].find(":") + 1: data[3].find(",")]))
        gtsfm_metrics.append(GtsfmMetric('Mean Observations Per Image ' + pipeline_name,
        ...
        with open(fpath, "r") as f:
            data = f.readlines()
            images = data[2:]
        gtsfm_metrics.append(GtsfmMetric('Number of Cameras ' + pipeline_name, data[2][data[2].rindex(":") + 1:]))

    comparison_metrics = GtsfmMetricsGroup(
        "GTSFM vs. COLMAP", gtsfm_metrics
    )
    comparison_metrics.save_to_json(os.path.join(json_path, 'comparison_metrics.json'))
