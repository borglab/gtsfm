...
class DataAssociation(NamedTuple):
    ...
    def run_da(
        ...
        logger.debug("Input number of tracks: %d", len(tracks_2d))
        logger.debug("Input avg. track length: %d", np.mean(track_lengths_2d))
        ...
        logger.debug("Classified  %d tracks", len(exit_codes_wrt_gt))
        ...
        logger.debug("Added %d tracks", triangulated_data.number_tracks())
        ...
        logger.debug("Computed track_cheirality_failure_ratio: %.2f", track_cheirality_failure_ratio)
