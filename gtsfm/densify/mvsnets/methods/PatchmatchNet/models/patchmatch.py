...
class DepthInitialization(nn.Module):
    ...
    def __init__(self, patchmatch_num_sample: int = 1):
        """init function
        Args:
...
class Propagation(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            grid: grid for bilinear gridding,
...
class PatchMatch(nn.Module):
    ...
    def get_propagation_grid(
        self, batch: int, height: int, width: int, offset: int, device: torch.device, img: torch.Tensor = None
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        if self.random_initialization:
            ...
            depth_sample = self.depth_initialization(
                True, depth_min, depth_max, height, width, self.patchmatch_interval_scale, device
            )
            ...
            depth_sample = self.depth_initialization(
                False, depth_min, depth_max, height, width, self.patchmatch_interval_scale, device, depth
            )
            del depth
...
class SimilarityNet(nn.Module):
    ...
    def __init__(self, G: int, neighbors: int = 9):
        """initialize
        Args:
            G: group number
...
class FeatureWeightNet(nn.Module):
    ...
    def __init__(self, num_feature: int, neighbors: int = 9, G: int = 8):
        """initialize
        Args:
