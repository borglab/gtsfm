...
class OneWayMatcher(MatcherBase):
    """Simple matcher using OpenCV, with optional ratio test"""
    ...
    def __perform_matching(self, descriptors_1: np.ndarray, descriptors_2: np.ndarray) -> np.ndarray:
        ...
        match_indices = np.array(
            [(idx1, idx2) for idx1, idx2 in match_indices_1to2.items()],
            dtype=np.uint32,
        )
        return match_indices
