...
def run_bundle_adjustment(
    ...
    for stage, basin in enumerate(per_stage_basins):
        ...
        post_ba_data, final_error = ba.run_simple_ba(
            pre_ba_data_optim, verbose=True, factor_graph_output_path=factor_graph_output_path
        )
    ...
    if run_triangulation_ba:
        ...
        tri_factor_graph_path = None
        if factor_graph_output_path is not None:
            tri_factor_graph_path = str(Path(factor_graph_output_path).with_name("factor_graph_triangulated.txt"))
        triangulated_post_ba_data, tri_final_error = ba.run_simple_ba(
            triangulated_pre_ba, verbose=True, factor_graph_output_path=tri_factor_graph_path
        )
