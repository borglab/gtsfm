...
class DataParserConfig:
    ...
    orientation_method: Literal["up"] = "up"
    center_method: Literal["poses", "none"] = "poses"
...
class JsonParser:
    """Parser for 'transforms.json' files.
    Args:
        data_dir: The nerfstudio_input directory with images folder and transformers.json and points3D.txt (optional) file
        test_every: A variable to create your left out validation set
    """
    def __init__(
        self,
        data_dir: str,
        ...
        self.image_paths = [self.data_dir / f for f in self.image_names]
        ...
        points3d_path = self.data_dir.parent / "ba_output/points3D.txt"
        if points3d_path.exists():
            print(f"Loading 3D points from {points3d_path}")
            try:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 7:
                        points_list.append([float(parts[1]), float(parts[2]), float(parts[3])])
                        colors_list.append([float(parts[4])/255.0, float(parts[5])/255.0, float(parts[6])/255.0])
                
                if len(points_list) > 0:
                    self.points = np.array(points_list, dtype=np.float32)
                    self.point_colors = np.array(colors_list, dtype=np.float32)
                    print(f"Successfully loaded {self.points.shape[0]} points and colors.")
                else:
                    print("points3D.txt was empty or malformed.")

            except Exception as e:
                print(f"Could not load points3D.txt: {e}")
