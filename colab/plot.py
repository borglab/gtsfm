...
def plot_points(fig, pts, color="rgba(255, 0, 0, 1)", ps=2, colorscale=None, name=None):
...
def plot_cameras(fig, reconstruction, **kwargs):
...
def plot_reconstruction(
    ...
    """Plot the 3D points and cameras from a COLMAP reconstruction."""
    # Filter outliers
...
def plot(colmap_results_folder):
    # Initialize the figure
    fig = init_figure()

    # Load COLMAP reconstruction using pycolmap
    reconstruction = pycolmap.Reconstruction(colmap_results_folder)

    # Plot the reconstruction (3D points and cameras)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot COLMAP reconstruction.")
    parser.add_argument("colmap_results_folder", type=str, help="Path to the COLMAP results folder.")
    args = parser.parse_args()

    plot(args.colmap_results_folder)
